...
![Lotto Quant Tool Dashboard](docs/dashboard-preview.svg)

上圖為目前 Web Dashboard 的介面預覽；實際顯示的號碼、百分比、遺漏期數、冷熱狀態、開獎日期與 Feature Importance 會隨 `data.json` 每次更新而改變。

### 畫面說明

| 畫面區塊 | 說明 |
|---|---|
| **最後更新時間** | 顯示目前 `data.json` 的產生／更新時間，方便確認資料新鮮度。 |
| **彩券卡片** | 大樂透、威力彩、今彩539，以及 3星彩、4星彩各自獨立顯示分析結果。 |
| **號碼與百分比** | 顯示 Random Forest 對各號碼產生的模型分類分數／機率輸出，並依分數排序。 |
| **🔥 熱／❄️ 冷** | 依近期 20 期的出現次數標示相對冷熱狀態，僅為歷史統計資訊。 |
| **漏 N 期** | 表示該號碼距離最近一次出現已經過多少期。 |
| **特別號／第二區** | 大樂透顯示歷史資料中的特別號統計結果；威力彩顯示第二區號碼。 |
| **Feature Importance** | 顯示 Random Forest 在目前資料中對近期開出狀態與頻率特徵的相對依賴程度。 |
| **下期開獎日期** | 依不同彩券的開獎週期與台北時間計算下一次預定開獎日。 |

> [!NOTE]
> Dashboard 顯示的百分比應理解成**模型在目前歷史資料與特徵設計下產生的分類輸出分數**，不是下一期真正開出該號碼的客觀中獎機率。

> [!IMPORTANT]
> `3星彩` 與 `4星彩` 目前仍為隨機模擬資料展示，並非以台灣彩券真實歷史資料訓練所得的預測結果。
...
├── docs/
│   └── dashboard-preview.svg  # README 系統介面預覽圖
├── data.json                  # 前端顯示用分析結果
